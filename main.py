...
class Satellite_Bot:
    ...
    def plans_txt_list(self, passes_dict):
        arr = ['noaa-15', 'noaa-18', 'noaa-19', 'meteor_m2', 'meteor_m2_2']
        self.plans = open('plans.txt', 'w')
        for n in arr:
            for i in passes_dict[n]:
                self.plans.write(f'{n}\n')
                for a in i:
                    self.plans.write(f'{a}\n')
                self.plans.write('----------\n')

    def loger_start_proger(self):
        self.log = open('log.txt', 'a+')
        self.log.write('Programm_Start: ' + str(datetime.today())[:-7] + '\n')
        self.log.close()

    def loger_stop_proger(self):
        self.log = open('log.txt', 'a+')
